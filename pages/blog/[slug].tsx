...
const Post = ({ post, related }: PostProps): JSX.Element => {
  ...
  const seoTitle = `${post.title} | Snail Run`
  const seoDesc = `${post.summary}`
  const url = `http://www.zhaoguilv.fun/blog/${post.slug}`
  // const currUrl = `https://og-image.zhaoguilv.vercel.app/${encodeURIComponent(post.title)}?desc=${encodeURIComponent(
  //   seoDesc,
  // )}&theme=dark.png`
  return (
    <Page>
      <NextSeo
        ...
        openGraph={{
          ...
          images: [
            {
              url: post.og ? `http://www.zhaoguilv.fun${post.og}` : `https://blog-1304565468.cos.ap-shanghai.myqcloud.com/blog/logo.png`,
              alt: post.title,
            },
          ],
          ...
          article: {
            ...
            authors: ['http://www.zhaoguilv.fun'],
          },
        }}
        twitter={{
